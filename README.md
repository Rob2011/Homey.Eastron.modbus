Bring accurate energy monitoring from Eastron Modbus Energy meters into Homey Pro for real-time insights and smart flows.

'Eastron – Modbus' brings reliable energy measurements from Energy Meters with Modbus TCP directly into Homey Pro. Whether you want to monitor total power consumption, per-phase values, or energy production, this app delivers real-time data to your Homey Energy dashboard and ‘flows’.

Build smart flows based on live energy data, visualize consumption within the Homey Energy dashboard and optimize your energy management. ‘Energy Meter – Modbus’ turns Energy Meter with Modbus TCP gateway into a powerful part of your smart home.


Requirements:
- Homey Pro
- Energy Meter with modbus interface (or Modbus protocol over TCP)
- Modbus TCP gateway
  (find my homebuild interface here: https://github.com/Rob2011/ESP32.Modbus-TCP-gateway)


Supported devices at this moment:
- Eastron SDM72D-M
- Eastron SDM120CT
- Eastron SDM230
- Eastron SDM630 V3

Link to the Eastron Homey Pro app: https://homey.app/nl-nl/app/eastron.modbus/Eastron/

Link to Homey Community with info / support for this app: https://community.homey.app/t/app-pro-eastron-app-to-support-eastron-sdm-energy-meters-in-homey-pro/147339




