import { Energymeter } from '../eastron-sdm120';

const DEFAULT_RETRY_INTERVAL = 20 * 1000; // Default 28 seconds

// SDM120CT Device class extending Energymeter
module.exports = class MyDevice extends Energymeter {
  ...
  async onInit() {
    this.log('SDM120CT Device has been initialized');
  ...
  async onAdded() {
    this.log('SDM120CT Device has been added');
  }
  ...
  async onSettings({
    ...
    this.log("SDM120CT Device settings were changed");
  ...
  async onRenamed(name: string) {
    this.log('SDM120CT Device was renamed');
  }

  // onDeleted is called when the user deleted the device.
  async onDeleted() {
    this.log('SDM120CT Device has been deleted');
    this.homey.clearInterval(this.timer);
  }
