import { Energymeter } from '../eastron-sdm230';

const DEFAULT_RETRY_INTERVAL = 20 * 1000; // Default 28 seconds

// SDM230 Device class extending Energymeter
module.exports = class MyDevice extends Energymeter {
  ...
  async onInit() {
    this.log('SDM230 Device has been initialized');
  ...
  async onAdded() {
    this.log('SDM230 Device has been added');
  }
  ...
  async onSettings({
    ...
    this.log("SDM230 Device settings were changed");
  ...
  async onRenamed(name: string) {
    this.log('SDM230 Device was renamed');
  }

  // onDeleted is called when the user deleted the device.
  async onDeleted() {
    this.log('SDM230 Device has been deleted');
    this.homey.clearInterval(this.timer);
  }
